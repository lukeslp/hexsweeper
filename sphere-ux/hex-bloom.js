...
(function (global) {
  ...
  const ROTATABLE_MENU = FACE_TAKEOVER || SCENE_STUDY;
  const FIXED_CENTER =
    DOCK_MODE === "center" ||
    FACE_TAKEOVER ||
    EQUATOR_RIBBON ||
  ...
  function foldMs() {
    if (prefersReducedMotion()) return 0;
    if (EGG_SHELL) return 500;
    if (INNER_CHAMBER) return 680;
  ...
  function initHexBloom(game) {
    ...
    function expand() {
      ...
      setPhase("open");
      if (EGG_SHELL) animateEggShell(1, 620);
      if (INNER_CHAMBER) animateChamber(1, 920);
    ...
    // Non-spatial profiles retain the modal scrim. Rotatable profiles pass
    // through to the canvas and receive the equivalent background-tap event.

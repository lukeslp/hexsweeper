/*
 * Hex bloom menu for the faceted polyhedron game.
 *
...
(function (global) {
  ...
  const DOCK_MODE = (() => {
    try {
      return (
        new URLSearchParams(location.search).get("dock") ||
        global.HEXSWEEPER_SPHERE_DOCK_MODE ||
        "center"
      ).replace(/[;,]+$/, "");
    } catch (_) {
      return "center";
  ...
  const FACE_TAKEOVER = DOCK_MODE === "faces";
  const EQUATOR_RIBBON = DOCK_MODE === "equator";
  const EGG_SHELL = DOCK_MODE === "eggshell" || DOCK_MODE === "egg";
  const INNER_CHAMBER = DOCK_MODE === "chamber";
  const POLAR_TAKES = ["capstan", "crown", "drill", "lid"];
  const POLAR_TAKE = POLAR_TAKES.includes(DOCK_MODE) ? DOCK_MODE : null;
  const POLAR_STUDY = !!POLAR_TAKE;
  const SPATIAL_SHELL = EGG_SHELL || INNER_CHAMBER;
  const SCENE_STUDY = SPATIAL_SHELL || POLAR_STUDY;
  const FIXED_CENTER =
    DOCK_MODE === "center" ||
    FACE_TAKEOVER ||
    EQUATOR_RIBBON ||
  ...
  const DOCK_CENTER = FIXED_CENTER || DOCK_CONTEXT;
  ...
  const ICONS = {
    ...
    // Panel-density icons: the pentagon remains the fixed macro topology;
    // the count tells how many playable micro-cells cover each face.
    sizeP19:
      '<path d="M12 2.8 20.8 9.2 17.4 19.5H6.6L3.2 9.2 12 2.8Z"/>' +
      '<text x="12" y="14.8" text-anchor="middle" fill="currentColor" stroke="none" font-family="IBM Plex Sans, sans-serif" font-size="7.2" font-weight="700">19</text>',
    sizeP37:
      '<path d="M12 2.2 21.5 9.1 17.9 20.2H6.1L2.5 9.1 12 2.2Z"/>' +
      '<path d="M12 6.2 17.2 10l-2 6.1H8.8L6.8 10 12 6.2Z" opacity=".28"/>' +
      '<text x="12" y="14.8" text-anchor="middle" fill="currentColor" stroke="none" font-family="IBM Plex Sans, sans-serif" font-size="7.2" font-weight="700">37</text>',
    sizeP61:
      '<path d="M12 1.2 22.2 8.6 18.3 20.7H5.7L1.8 8.6 12 1.2Z"/>' +
      '<path d="M12 4.8 18.8 9.7l-2.6 8H7.8l-2.6-8L12 4.8Z" opacity=".24"/>' +
      '<path d="M12 8.2 15.6 10.8l-1.4 4.3H9.8l-1.4-4.3L12 8.2Z" opacity=".24"/>' +
      '<text x="12" y="14.7" text-anchor="middle" fill="currentColor" stroke="none" font-family="IBM Plex Sans, sans-serif" font-size="7" font-weight="700">61</text>',
    // Legacy glyphs remain for archived query variants copied with the lane.
  ...
  function foldMs() {
    if (prefersReducedMotion()) return 0;
    if (EGG_SHELL) return 220;
    if (INNER_CHAMBER) return 460;
    if (POLAR_STUDY) return POLAR_TAKE === "lid" ? 420 : 300;
  ...
  function initHexBloom(game) {
    ...
    const shellScene = SCENE_STUDY ? document.createElement("div") : null;
    if (shellScene) {
      shellScene.className = "hb-shell-scene";
      shellScene.setAttribute("aria-hidden", "true");
      root.appendChild(shellScene);
    }
      ...
      if (POLAR_STUDY) {
        ...
        if (disk && disk.r) {
          const targetY =
            rect.top +
            disk.cy -
            disk.r * (POLAR_TAKE === "capstan" ? 0.32 : POLAR_TAKE === "lid" ? 0.28 : 0.34);
          return {
            cx: Math.max(90, Math.min(w - 90, rect.left + disk.cx)),
            cy: Math.max(112, Math.min(h - 160, targetY)),
          };
        }
      }
    ...
    function cycleSize() {
      ...
      announce("Panel density · " + SIZES[next][1]);
    }
    ...
    function categories() {
      ...
      const all = [
        ...
        },
        {
          ...
          badge: true,
          aria: () =>
            "Panel density: " + currentSize() + ". Activate to cycle.",
          cycle: cycleSize,
        },
        {
          id: "surface",
      ...
      if (SHARED_MENU) {
        const menu = [
          ...
          {
            id: "size",
            label: () => "Panel " + currentSize(),
            icon: sizeIcon,
            iconOnly: true,
            aria: () =>
              "Panel density " + currentSize() + ". Activate to cycle P19, P37, P61.",
            cycle: cycleSize,
          },
        ];
    ...
    let lastFocus = null;
    let ribbonAngle = Math.PI / 2;
    let ribbonActions = [];
    let ribbonSuppressClickUntil = 0;
    let eggProgress = 0;
    let eggRaf = 0;
    ...
    function buildEggShellScene() {
      if (!shellScene) return false;
      shellScene.textContent = "";
      shellScene.dataset.kind = "eggshell";
      document.body.classList.add("hex-bloom-eggshell-active");
      if (typeof game.setChromeShell === "function") {
        game.setChromeShell("eggshell", eggProgress);
      }
      return true;
    }
    ...
    function buildChamberScene() {
      if (!shellScene) return false;
      shellScene.textContent = "";
      shellScene.dataset.kind = "chamber";
      document.body.classList.add("hex-bloom-chamber-active");
      if (typeof game.setChromeProjection === "function") {
        game.setChromeProjection("chamber", chamberProgress);
      }
      return true;
    }

    function buildPolarScene() {
      if (!shellScene || !POLAR_STUDY) return false;
      const disk =
        typeof game.getSphereScreen === "function" ? game.getSphereScreen() : null;
      if (!disk || !disk.r) return false;
      const { cx, cy } = centerXY();
      const { HEX } = tileMetrics();
      shellScene.textContent = "";
      shellScene.dataset.kind = "polar-" + POLAR_TAKE;
      shellScene.style.setProperty("--polar-cx", cx.toFixed(1) + "px");
      shellScene.style.setProperty("--polar-cy", cy.toFixed(1) + "px");
      shellScene.style.setProperty("--polar-r", disk.r.toFixed(1) + "px");
      shellScene.style.setProperty("--polar-unit", HEX.toFixed(1) + "px");

      const object = document.createElement("div");
      object.className = "hb-polar-object hb-polar-object--" + POLAR_TAKE;
      if (POLAR_TAKE === "capstan") {
        ["collar", "shaft", "top", "seam-left", "seam-right"].forEach((part) => {
          const node = document.createElement("div");
          node.className = "hb-polar-" + part;
          object.appendChild(node);
        });
      } else if (POLAR_TAKE === "crown") {
        const collar = document.createElement("div");
        collar.className = "hb-polar-collar";
        object.appendChild(collar);
        for (let i = 0; i < 6; i += 1) {
          const petal = document.createElement("div");
          petal.className = "hb-polar-petal";
          petal.style.setProperty("--petal-i", String(i));
          object.appendChild(petal);
        }
      } else if (POLAR_TAKE === "drill") {
        for (let i = 0; i < 3; i += 1) {
          const ring = document.createElement("div");
          ring.className = "hb-polar-ring hb-polar-ring--" + (i + 1);
          object.appendChild(ring);
        }
        const bore = document.createElement("div");
        bore.className = "hb-polar-bore";
        object.appendChild(bore);
      } else if (POLAR_TAKE === "lid") {
        ["hinge", "tether", "panel", "panel-rim"].forEach((part) => {
          const node = document.createElement("div");
          node.className = "hb-polar-" + part;
          object.appendChild(node);
        });
      }
      shellScene.appendChild(object);
      return true;
    }

    function clearSpatialScene() {
      cancelAnimationFrame(eggRaf);
      eggRaf = 0;
      eggProgress = 0;
      cancelAnimationFrame(chamberRaf);
      chamberRaf = 0;
      chamberProgress = 0;
      if (typeof game.setChromeProjection === "function") {
        game.setChromeProjection(null, 0);
      }
      if (typeof game.setChromeShell === "function") {
        game.setChromeShell(null, 0);
      }
      if (shellScene) {
        shellScene.textContent = "";
        delete shellScene.dataset.kind;
      }
      document.body.classList.remove(
        "hex-bloom-eggshell-active",
        "hex-bloom-chamber-active"
      );
    }

    function animateEggShell(target, duration) {
      if (!EGG_SHELL || typeof game.setChromeShell !== "function") return;
      cancelAnimationFrame(eggRaf);
      eggRaf = 0;
      const to = Math.max(0, Math.min(1, target));
      const from = eggProgress;
      const paint = (progress) => {
        eggProgress = progress;
        game.setChromeShell(progress > 0 || to > 0 ? "eggshell" : null, progress);
      };
      if (prefersReducedMotion() || !duration || from === to) {
        paint(to);
        return;
      }
      const effectiveDuration = Math.max(1, duration * Math.abs(to - from));
      const started = performance.now();
      const tick = (now) => {
        const raw = Math.max(
          0,
          Math.min(1, (now - started) / effectiveDuration)
        );
        paint(from + (to - from) * raw);
        if (raw < 1) eggRaf = requestAnimationFrame(tick);
        else eggRaf = 0;
      };
      eggRaf = requestAnimationFrame(tick);
    }

    function animateChamber(target, duration) {
      if (!INNER_CHAMBER || typeof game.setChromeProjection !== "function") return;
      cancelAnimationFrame(chamberRaf);
      chamberRaf = 0;
      const to = Math.max(0, Math.min(1, target));
      const from = chamberProgress;
      const paint = (progress) => {
        chamberProgress = progress;
        game.setChromeProjection(
          progress > 0 || to > 0 ? "chamber" : null,
          progress
        );
      };
      ...
      const tick = (now) => {
        ...
        // The renderer supplies the one smoothstep. Feeding it linear time
        // keeps useful motion in every frame instead of double-easing the turn.
        paint(from + (to - from) * raw);
        if (raw < 1) chamberRaf = requestAnimationFrame(tick);
        else chamberRaf = 0;
      };
      chamberRaf = requestAnimationFrame(tick);
    }

    function prepareSpatialScene() {
      if (EGG_SHELL) return buildEggShellScene();
      if (INNER_CHAMBER) return buildChamberScene();
      if (POLAR_STUDY) return buildPolarScene();
      return false;
    }

    function refreshSpatialSceneSoon() {
      if (!EGG_SHELL && !INNER_CHAMBER) return;
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (state.phase !== "open") return;
          if (EGG_SHELL) buildEggShellScene();
          else if (INNER_CHAMBER) buildChamberScene();
        });
      });
    }
        ...
        if (SPATIAL_SHELL && id === "size") {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              ...
              bloomPetals();
              if (EGG_SHELL) buildEggShellScene();
              if (INNER_CHAMBER) buildChamberScene();
            });
          });
        } else {
          refreshSpatialSceneSoon();
        }
        return;
      }
    ...
    function makeExperimentAction(cat, index, extraClass) {
      ...
      const el = makeHex({
        cls:
          ...
          " cat-" +
          cat.id +
          (iconName === "flag" && cat.id !== "flagmode" ? " is-flag-icon" : "") +
          (cat.id === "flagmode" ? " is-flag-toggle" : "") +
          (flagModeActive ? " is-mode-active" : ""),
        label,
        ariaLabel: typeof cat.aria === "function" ? cat.aria() : label,
        icon: iconName,
        iconOnly: true,
        tooltip: label,
      });
      ...
      el.addEventListener("click", (event) => {
        if (EQUATOR_RIBBON && Date.now() < ribbonSuppressClickUntil) {
        ...
        if (typeof cat.cycle === "function") cat.cycle();
        refreshPrimaryLabels();
        announce(typeof cat.aria === "function" ? cat.aria() : label);
        if (cat.id === "size" && (FACE_TAKEOVER || EQUATOR_RIBBON || POLAR_STUDY)) {
          requestAnimationFrame(() => {
            if (state.phase !== "open") return;
            sizeCenterFlower();
            if (POLAR_STUDY) buildPolarScene();
            renderBloom({ petals: true });
            bloomPetals();
          });
        }
    ...
    function makeExperimentReset(extraClass) {
      const el = makeHex({
        cls: "is-primary is-center-reset " + (extraClass || ""),
        label: "Reset",
    ...
    function placeProjectedFace(el, face, canvasRect) {
      ...
      el.style.setProperty("--face-icon", Math.max(22, Math.min(46, height * 0.5)).toFixed(1) + "px");
    }

    function renderFaceTakeover(cats) {
      const patch =
        typeof game.getMenuPatch === "function" ? game.getMenuPatch() : null;
      const canvas = document.getElementById("gameCanvas");
      if (!patch || !canvas) return false;
      const canvasRect = canvas.getBoundingClientRect();
      const ring = orderProjectedRing(patch);
      ring.forEach((face, index) => {
        const el = makeExperimentAction(cats[index], index, "hb-face-action");
        placeProjectedFace(el, face, canvasRect);
        el.classList.add("folded");
        el.dataset.delay = String(45 + index * 38);
        stage.appendChild(el);
      });
      const reset = makeExperimentReset("hb-face-action hb-face-center");
      placeProjectedFace(reset, patch.center, canvasRect);
    ...
    function layoutEquatorRibbon() {
      if (!ribbonActions.length) return;
      const disk =
        typeof game.getSphereScreen === "function" ? game.getSphereScreen() : null;
      if (!disk || !disk.r) return;
      const canvas = document.getElementById("gameCanvas");
      const canvasRect = canvas ? canvas.getBoundingClientRect() : { left: 0, top: 0 };
      const cx = canvasRect.left + disk.cx;
      const cy = canvasRect.top + disk.cy;
      const tileW = Math.max(46, Math.min(78, disk.r * 0.27));
      const tileH = tileW / 0.873;
      const ringX = disk.r * 0.78;
      const ringY = disk.r * 0.24;
      ribbonActions.forEach((el, index) => {
        const angle = ribbonAngle + (index * Math.PI * 2) / ribbonActions.length;
        const depth = (Math.sin(angle) + 1) / 2;
        const scale = 0.58 + depth * 0.46;
        el.style.left = (cx + Math.cos(angle) * ringX).toFixed(1) + "px";
        el.style.top = (cy + Math.sin(angle) * ringY).toFixed(1) + "px";
        el.style.bottom = "auto";
        el.style.width = tileW.toFixed(1) + "px";
        el.style.height = tileH.toFixed(1) + "px";
        el.style.setProperty("--ribbon-scale", scale.toFixed(3));
        el.style.setProperty("--ribbon-opacity", (0.28 + depth * 0.72).toFixed(3));
        el.style.setProperty("--ribbon-blur", ((1 - depth) * 0.65).toFixed(2) + "px");
        el.style.zIndex = String(20 + Math.round(depth * 100));
        el.style.pointerEvents = depth < 0.16 ? "none" : "auto";
      });
    }

    function renderEquatorRibbon(cats) {
      ribbonActions = [];
      const reset = makeExperimentReset("hb-ribbon-action");
      ribbonActions.push(reset);
      cats.forEach((cat, index) => {
        ribbonActions.push(
          makeExperimentAction(cat, index, "hb-ribbon-action")
        );
      });
      ribbonActions.forEach((el, index) => {
        el.classList.add("folded");
        el.dataset.delay = String(index * 28);
        stage.appendChild(el);
      });
      centerActionBtn = reset;
      layoutEquatorRibbon();
    ...
    function polarSeats(hex) {
      if (POLAR_TAKE === "capstan") {
        return {
          reset: { x: 0, y: -1.02 * hex },
          actions: [
            { x: -0.92 * hex, y: 0.48 * hex },
            { x: 0.92 * hex, y: 0.48 * hex },
            { x: -0.92 * hex, y: 1.98 * hex },
            { x: 0.92 * hex, y: 1.98 * hex },
            { x: -0.92 * hex, y: 3.48 * hex },
            { x: 0.92 * hex, y: 3.48 * hex },
          ],
        };
      }
      ...
      const ring = DIRECTIONS.map((direction) => {
        const point = hexToPixel(direction.q, direction.r, hex);
        return { x: point.x, y: point.y };
      });
      return { reset: { x: 0, y: 0 }, actions: ring };
    }

    function renderPolarStudy(cats) {
      const { HEX } = tileMetrics();
      const { cx, cy } = centerXY();
      const seats = polarSeats(HEX);
      cats.forEach((cat, index) => {
        const el = makeExperimentAction(
          cat,
          index,
          "hb-polar-action hb-polar-action--" + POLAR_TAKE
        );
        const seat = seats.actions[index];
        place(el, cx + seat.x, cy + seat.y, cx, cy);
        el.dataset.polarIndex = String(index);
        el.dataset.delay = String(55 + index * 34);
        el.classList.add("folded");
        stage.appendChild(el);
      });
      const reset = makeExperimentReset(
        "hb-polar-action hb-polar-reset hb-polar-action--" + POLAR_TAKE
      );
      place(reset, cx, cy + seats.reset.y, cx, cy);
      reset.dataset.delay = "0";
      reset.classList.add("folded");
      stage.appendChild(reset);
      centerActionBtn = reset;
    ...
    function renderBloom(options) {
      ...
      if (EQUATOR_RIBBON && renderEquatorRibbon(cats)) return;
      if (POLAR_STUDY && renderPolarStudy(cats)) return;
      ...
      cats.forEach((cat, i) => {
        ...
        el.dataset.delay = DOCK_CENTER
          ? EGG_SHELL
            ? String(12 + Math.round(34 * ringPhase))
            : INNER_CHAMBER
              ? String(24 + Math.round(60 * ringPhase))
              : String(
                  (DOCK_CONTEXT ? 18 : 110) +
                    Math.round((DOCK_CONTEXT ? 72 : 260) * ringPhase)
                )
          : String(i * 26);
        el.addEventListener("click", (e) => {
          e.preventDefault();
    ...
    function sizeCenterFlower() {
      if (!DOCK_CENTER || FACE_TAKEOVER || EQUATOR_RIBBON) return;
      const disk =
        typeof game.getSphereScreen === "function" ? game.getSphereScreen() : null;
      if (!disk || !disk.r) return;
      ...
      if (POLAR_STUDY) {
        const widthUnits = POLAR_TAKE === "drill" ? 7.1 : POLAR_TAKE === "crown" ? 5.5 : 4.6;
        const heightUnits = POLAR_TAKE === "capstan" ? 5.6 : POLAR_TAKE === "drill" ? 6.4 : 4.8;
        const desired =
          disk.r *
          (POLAR_TAKE === "capstan" ? 0.15 : POLAR_TAKE === "drill" ? 0.135 : 0.17);
        const hex = Math.max(
          29,
          Math.min(66, desired, (stageWidth - 24) / widthUnits, (stageHeight - 104) / heightUnits)
        );
      ...
      const viewportCap = DOCK_CONTEXT
        ...
        : 90;
      const spatialFit = EGG_SHELL ? 0.43 : INNER_CHAMBER ? 0.42 : FLOWER_FIT;
      const hex = Math.max(
        DOCK_CONTEXT ? (narrowContext ? 36 : 25) : 30,
        Math.min(
          DOCK_CONTEXT ? (narrowContext ? 52 : 66) : 90,
          viewportCap,
          ((disk.r * spatialFit) / FLOWER_OUTER_HEX) *
    ...
    function expand() {
      ...
      setPhase("open");
      if (EGG_SHELL) animateEggShell(1, 260);
      if (INNER_CHAMBER) animateChamber(1, 560);
    ...
    function collapse() {
      ...
      if (EGG_SHELL) animateEggShell(0, foldMs());
      if (INNER_CHAMBER) animateChamber(0, foldMs());
      setPhase("falling");
      ...
      const finish = () => {
        ...
        hub.hidden = true;
        if (SCENE_STUDY) clearSpatialScene();
    ...
    if (DOCK_CENTER && !FACE_TAKEOVER && !EQUATOR_RIBBON && !SCENE_STUDY) {
    ...
    if (DOCK_CENTER) {
      ...
      if (EQUATOR_RIBBON) root.dataset.equatorRibbon = "true";
      if (EGG_SHELL) root.dataset.eggShell = "true";
      if (INNER_CHAMBER) root.dataset.innerChamber = "true";
      if (POLAR_STUDY) {
        root.dataset.polarStudy = "true";
        root.dataset.polarTake = POLAR_TAKE;
      }
    ...
    if (EQUATOR_RIBBON) {
      ...
      stage.addEventListener("pointerdown", (event) => {
        if (state.phase !== "open") return;
        const tile = event.target.closest && event.target.closest(".hb-ribbon-action");
        if (!tile) return;
        ribbonDrag = {
        ...
        root.dataset.ribbonDragging = "true";
      });
      stage.addEventListener("pointermove", (event) => {
        if (!ribbonDrag || ribbonDrag.pointerId !== event.pointerId) return;
        const dx = event.clientX - ribbonDrag.x;
        ribbonDrag.x = event.clientX;
        ribbonDrag.distance += Math.abs(dx);
        ribbonAngle += dx * 0.012;
        layoutEquatorRibbon();
        if (ribbonDrag.distance > 4) event.preventDefault();
      });
      const finishRibbonDrag = (event) => {
        if (!ribbonDrag || ribbonDrag.pointerId !== event.pointerId) return;
        if (ribbonDrag.distance > 5) {
          ribbonSuppressClickUntil = Date.now() + 180;
        }
        try {
          stage.releasePointerCapture(event.pointerId);
        } catch (_) {}
        ribbonDrag = null;
        delete root.dataset.ribbonDragging;
      };
      stage.addEventListener("pointerup", finishRibbonDrag);
      stage.addEventListener("pointercancel", finishRibbonDrag);
    }
    ...
    window.addEventListener("resize", () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        if (
          !widthChanged &&
          !((DOCK_CONTEXT || SCENE_STUDY) && heightChanged)
        )
          return;
        if (state.phase === "open") {
